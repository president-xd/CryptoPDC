...
void SHA256::compute(const uint8_t* input, size_t length, uint8_t* output) {
    uint32_t h[8] = {
    ...
    for (size_t offset = 0; offset < new_len; offset += 64) {
        ...
        // Copy chunk to w[0..15] (big-endian)
        for (int i = 0; i < 16; i++) {
...
byte_vector SHA256::hash(const byte_vector& input) const {
    byte_vector result(32);
    compute(input.data(), input.size(), result.data());
    return result;
}

byte_vector SHA256::hash(const std::string& input) const {
    byte_vector result(32);
    compute(reinterpret_cast<const uint8_t*>(input.data()), input.size(), result.data());
    return result;
}

bool SHA256::verify(const std::string& input, const byte_vector& target_hash) const {
