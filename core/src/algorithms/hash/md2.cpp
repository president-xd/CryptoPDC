#include "cryptopdc/algorithms/hash/md2.hpp"
#include <cstring>
#include <cstdint>

namespace cryptopdc {
namespace algorithms {
namespace hash {

// MD2 S-box (pi substitution table)
static const uint8_t MD2_S[256] = {
     41, 46, 67, 201, 162, 216, 124,  1,  61, 54,  84, 161, 236, 240,  6,  19,
     98, 167,  5, 243, 192, 199, 115, 140, 152, 147, 43, 217, 188, 76, 130, 202,
     30, 155,  87, 60, 253, 212, 224,  22, 103,  66, 111,  24, 138,  23, 229, 18,
    190,  78, 196, 214, 218, 158, 222,  73, 160, 251, 245, 142, 187,  47, 238, 122,
    169, 104, 121, 145,  21, 178,   7,  63, 148, 194,  16,  137,  11,  34, 95,  33,
    128, 127,  93, 154,  90, 144,  50,  39, 53,  62, 204, 231, 191, 247, 151,  3,
    255,  25,  48, 179,  72, 165, 181, 209, 215,  94, 146,  42, 172,  86, 170, 198,
     79, 184,  56, 210, 150, 164, 125, 182, 118, 252, 107, 226, 156, 116,  4, 241,
     69, 157, 112,  89, 100, 113, 135,  32, 134,  91, 207, 101, 230,  45, 168,  2,
     27,  96,  37, 173, 174, 176, 185, 246,  28,  70,  97, 105,  52,  64, 126, 15,
     85,  71, 163,  35, 221,  81, 175,  58, 195,  92, 249, 206, 186, 197, 234,  38,
     44,  83,  13, 110, 133,  40, 132,   9, 211, 223, 205, 244,  65, 129,  77, 82,
    106, 220,  55, 200, 108, 193, 171, 250,  36, 225, 123,   8,  12, 189, 177,  74,
    120, 136, 149, 139, 227,  99, 232, 109, 233, 203, 213, 254,  59,   0,  29,  57,
    242, 239, 183,  14, 102,  88, 208, 228, 166, 119, 114, 248, 235, 117,  75,  10,
     49,  68,  80, 180, 143, 237,  31,  26, 219, 153, 141,  51, 159,  17, 131, 20
};

MD2::MD2() {}

void MD2::compute(const uint8_t* input, size_t length, uint8_t* output) {
    uint8_t X[48];
    uint8_t C[16];
    uint8_t M[16];
    
    memset(X, 0, 48);
    memset(C, 0, 16);
    
    // Padding
    size_t padLen = 16 - (length % 16);
    size_t paddedLen = length + padLen;
    uint8_t* padded = new uint8_t[paddedLen + 16]; // +16 for checksum
    memcpy(padded, input, length);
    
    for (size_t i = length; i < paddedLen; i++) {
        padded[i] = (uint8_t)padLen;
    }
    
    // Compute checksum
    uint8_t L = 0;
    for (size_t i = 0; i < paddedLen; i += 16) {
        for (int j = 0; j < 16; j++) {
            C[j] ^= MD2_S[padded[i + j] ^ L];
            L = C[j];
        }
    }
    
    // Append checksum to message
    memcpy(padded + paddedLen, C, 16);
    paddedLen += 16;
    
    // Process message blocks
    for (size_t i = 0; i < paddedLen; i += 16) {
        // Copy block into X
        for (int j = 0; j < 16; j++) {
            M[j] = padded[i + j];
            X[16 + j] = M[j];
            X[32 + j] = X[j] ^ M[j];
        }
        
        // Do 18 rounds
        uint8_t t = 0;
        for (int round = 0; round < 18; round++) {
            for (int j = 0; j < 48; j++) {
                X[j] ^= MD2_S[t];
                t = X[j];
            }
            t = (t + round) & 0xFF;
        }
    }
    
    delete[] padded;
    
    // Output hash (first 16 bytes of X)
    memcpy(output, X, 16);
}

byte_vector MD2::hash(const byte_vector& input) const {
    byte_vector result(16);
    compute(input.data(), input.size(), result.data());
    return result;
}

byte_vector MD2::hash(const std::string& input) const {
    byte_vector result(16);
    compute(reinterpret_cast<const uint8_t*>(input.data()), input.size(), result.data());
    return result;
}

bool MD2::verify(const std::string& input, const byte_vector& target_hash) const {
    auto computed = hash(input);
    return computed == target_hash;
}

} // namespace hash
} // namespace algorithms
} // namespace cryptopdc
