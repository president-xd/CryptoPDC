#include "cryptopdc/cuda/hash/ntlm_kernel.cuh"
...
// MD4 functions for NTLM
#define MD4_F(x, y, z) (((x) & (y)) | ((~x) & (z)))
#define MD4_G(x, y, z) (((x) & (y)) | ((x) & (z)) | ((y) & (z)))
#define MD4_H(x, y, z) ((x) ^ (y) ^ (z))

#define MD4_ROUND1(a, b, c, d, x, s) a = rotl((a + MD4_F(b, c, d) + x), s)
#define MD4_ROUND2(a, b, c, d, x, s) a = rotl((a + MD4_G(b, c, d) + x + 0x5A827999), s)
#define MD4_ROUND3(a, b, c, d, x, s) a = rotl((a + MD4_H(b, c, d) + x + 0x6ED9EBA1), s)

__device__ void ntlm_hash_device(const uint8_t* input, size_t length, uint8_t* output) {
    ...
    size_t new_len = ((((utf16_len + 8) / 64) + 1) * 64) - 8;
    
    msg[utf16_len] = 0x80;
    for (size_t i = utf16_len + 1; i < new_len; i++) msg[i] = 0;
    
    uint64_t bits_len = utf16_len * 8;
...
__global__ void ntlm_crack_kernel(
    ...
    ntlm_hash_device((const uint8_t*)key, key_length, computed_hash);
...
cudaError_t launch_ntlm_crack(
    ...
    ntlm_crack_kernel<<<num_blocks, threads_per_block>>>(
