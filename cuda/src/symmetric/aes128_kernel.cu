...
// AES S-box in constant memory for fast access
...
__device__ __forceinline__ uint8_t gf_mul2(uint8_t a) {
    return (a << 1) ^ ((a & 0x80) ? 0x1b : 0x00);
}

// Galois field multiplication by 3
__device__ __forceinline__ uint8_t gf_mul3(uint8_t a) {
    return gf_mul2(a) ^ a;
}

// AES-128 key expansion (11 round keys, 176 bytes total)
__device__ void aes128_key_expansion(const uint8_t* key, uint8_t* round_keys) {
    // Copy original key
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        round_keys[i] = key[i];
    ...
    while (bytes_generated < 176) {
        ...
        if (bytes_generated % 16 == 0) {
            // RotWord
            uint8_t t = temp[0];
            temp[0] = d_sbox[temp[1]];
            temp[1] = d_sbox[temp[2]];
            temp[2] = d_sbox[temp[3]];
            temp[3] = d_sbox[t];
            
            temp[0] ^= d_rcon[rcon_idx++];
        }
        
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            round_keys[bytes_generated + i] = round_keys[bytes_generated - 16 + i] ^ temp[i];
        }
        bytes_generated += 4;
    }
}
...
__device__ void aes128_encrypt_block(const uint8_t* plaintext, const uint8_t* key, uint8_t* ciphertext) {
    uint8_t state[16];
...
// Kernel to crack AES-128 keys
__global__ void aes128_crack_kernel(
    ...
    const uint8_t* key = candidate_keys + idx * 16;
    uint8_t computed_ciphertext[16];
    
    // Encrypt with candidate key
    aes128_encrypt_block(plaintext, key, computed_ciphertext);
    
    // Check if matches expected ciphertext
    bool match = true;
    #pragma unroll
    for (int i = 0; i < 16; i++) {
...
static void hex_to_bytes(const std::string& hex, uint8_t* bytes, size_t max_len) {
    ...
    // Pad with zeros if hex string is shorter
...
std::string cuda_crack_aes128(
    ...
    for (size_t batch_start = 0; batch_start < candidates.size() && result.empty(); batch_start += BATCH_SIZE) {
        ...
        std::vector<uint8_t> h_keys(batch_count * 16);
        for (size_t i = 0; i < batch_count; i++) {
            hex_to_bytes(candidates[batch_start + i], h_keys.data() + i * 16, 16);
        }
        
        // Allocate and copy keys to device
        err = cudaMalloc(&d_keys, batch_count * 16);
        if (err != cudaSuccess) break;
        
        cudaMemcpy(d_keys, h_keys.data(), batch_count * 16, cudaMemcpyHostToDevice);
        ...
        for (size_t chunk_start = 0; chunk_start < batch_count; chunk_start += (size_t)num_blocks * threads_per_block) {
            ...
            aes128_crack_kernel<<<chunk_blocks, threads_per_block>>>(
                d_plaintext,
                d_ciphertext,
                d_keys + chunk_start * 16,
                chunk_size,
                d_found_idx
            );
...
bool cuda_aes128_available() {
    int device_count = 0;
    cudaError_t err = cudaGetDeviceCount(&device_count);
    return (err == cudaSuccess && device_count > 0);
}

size_t cuda_aes128_batch_size() {
    return 1024 * 256;  // 256K candidates per batch
}
